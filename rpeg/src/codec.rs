...
use crate::dct_quantize::{dct_to_quantize, quantize_to_dct};
...
pub fn compress(filename: Option<&str>) {
    ...
    let out_width = comp_step2.width() as u32;
    let out_height = comp_step2.height() as u32;

    let comp_step3 = rgb_to_float(comp_step2);
    let comp_step4 = frgb_to_cv(comp_step3);
    let comp_step5 = cv_to_dct(comp_step4);
    let comp_step6 = dct_to_quantize(comp_step5);

    // Turn the dct to cv, component video to f32 rgb, turn f32 rgb to u16 rgb
    let decomp_step0 = quantize_to_dct(comp_step6);
    let decomp_step1 = dct_to_cv(decomp_step0);
    ...
    let out2 = new_rgb(out1, out_width, out_height as u32, img.denominator);
