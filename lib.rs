// Timestamp: 2021-01-03T18:00:00+08:00
...
impl<T: Clone + Debug> Array2<T> {
    ...
    pub fn set_array_zero( height: usize, width: usize, value: T) -> Self {
    ...
    /// Clone method for Array2
    pub fn clone(&self) -> Array2<T> {
        Array2 {
            width: self.width,
            height: self.height,
            data: self.data.iter().map(|row| row.clone()).collect(),
        }
    }
